...
export class MenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, 94, 'Wiz Kidz Conference  ✦  Tap pieces · Fewest moves · Beat your score', {
    ...
    const enterText = this.add.text(width / 2, 448, '● TAP THE BOARD OR PRESS ENTER ●', {
    ...
    // Primary: touchscreen / pointer. Fallback: Enter / Space key.
    this.input.on('pointerdown', (ptr: Phaser.Input.Pointer) => {
      if (ptr.y > height - 40) return; // avoid theme toggle area
      this.launchGame();
    });
